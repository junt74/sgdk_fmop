...
/** 試聴用 FM チャンネル（port 0 / ch 0〜2）。 */
#define FM_PREVIEW_YM_CH 2u
/** `fm_confirm_note_get()` が 0〜107 → MIDI 音高は +12（o0 C = MIDI 12）。 */
#define FM_PREVIEW_MIDI_BASE 12u
/** `FM_PREVIEW_PITCH` の要素数（MIDI 12〜119）。 */
...
static void write_ym_slot(u8 ch, u8 sl, u8 reg, u8 val)
{
    YM2612_write(0, reg | (u8)(sl * 4u) | ch);
    YM2612_write(1, val);
}

static void write_ym_ch(u8 ch, u8 reg, u8 val)
{
    YM2612_write(0, reg | ch);
    YM2612_write(1, val);
}
...
static void apply_slot_from_op(u8 ch, u8 sl, const FmOpParams *op)
{
    ...
    write_ym_slot(ch, sl, 0x30, r30);
    write_ym_slot(ch, sl, 0x40, (u8)(op->tl & 0x7Fu));
    write_ym_slot(ch, sl, 0x50, (u8)(((op->k & 3u) << 6) | (op->ar & 31u)));
    write_ym_slot(ch, sl, 0x60, (u8)(op->dr & 31u));
    write_ym_slot(ch, sl, 0x70, (u8)(op->sr & 31u));
    write_ym_slot(ch, sl, 0x80, (u8)(((op->sl & 15u) << 4) | (op->rr & 15u)));
    write_ym_slot(ch, sl, 0x90, (u8)(op->ssg_lo & 15u));
}
...
static void apply_patch_to_channel(const FmPatch *patch, u8 ch)
{
    ...
    write_ym_ch(ch, 0xB0, (u8)(((patch->fb & 7u) << 3) | (patch->alg & 7u)));
    write_ym_ch(ch, 0xB4, 0xC0);
...
static void apply_midi_pitch(u8 ch, u8 midi)
{
    ...
    write_ym_ch(ch, 0xA0, (u8)(fn & 0xFFu));
    write_ym_ch(ch, 0xA4, (u8)(((fn >> 8) & 3u) | (u8)((blk & 7u) << 2)));
}

static void key_off_ch(u8 ch) { YM2612_writeReg(0, 0x28, ch & 3u); }

static void key_on_ch(u8 ch) { YM2612_writeReg(0, 0x28, (u8)(0xF0u | (ch & 3u))); }
...
void fm_preview_frame(const FmPatch *patch, u16 joy, u16 joy_prev)
{
    ...
    if (!a)
    {
        if (a_prev)
        {
            const bool taken_in = Z80_getAndRequestBus(TRUE);
...

    {
        ...
        taken = Z80_getAndRequestBus(TRUE);
