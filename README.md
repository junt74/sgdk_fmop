...
| B ＋ 左右（B 押下中に左右を**新規に**押す） | 確認用ノート（画面 `NOTE:` 行）を **半音** ずつ変更（範囲は o0 C〜o8 B） |
| B ＋ 上下（B 押下中に上下を**新規に**押す） | 確認用ノートを **1 オクターブ** ずつ変更（同じ範囲でクランプ） |
| A ボタン（押下中） | 現在の音色で発音 |
| A ボタン（離す） | 消音 |

確認用ノートは A ボタン試聴時の基準音程に使われます（既定は o4 C）。
